...
function readStoredPanelSide(): DebugPanelSide {
  if (typeof window === "undefined") {
    return "right"
  }

  const savedSide = window.sessionStorage.getItem(DEBUG_PANEL_SIDE_KEY)
  return savedSide && isDebugPanelSide(savedSide) ? savedSide : "right"
}

function readStoredPanelTab(): DebugPanelTab {
  if (typeof window === "undefined") {
    return "overview"
  }

  const savedTab = window.sessionStorage.getItem(DEBUG_PANEL_TAB_KEY)
  return savedTab && isDebugPanelTab(savedTab) ? savedTab : "overview"
}

function readStoredAttached(): boolean {
  if (typeof window === "undefined") {
    return false
  }

  return window.sessionStorage.getItem(DEBUG_PANEL_ATTACH_KEY) === "true"
}

async function copyDebugText(value: string) {
  await navigator.clipboard.writeText(value)
}

const PANEL_SIDE_LAYOUTS: Record<DebugPanelSide, string> = {
  bottom: "inset-x-0 bottom-0 flex-col border-t",
  left: "top-0 left-0 h-full flex-col border-r",
  right: "top-0 right-0 h-full flex-col border-l",
}
...
function parsePluginLogMessage(message: string) {
  ...
  if (headerEnd === -1 || !message.startsWith("[")) {
    return null
  }

  const header = message.slice(1, headerEnd)
  const text = message.slice(headerEnd + 2)
  const parts = header.split("][")
  if (parts.length !== 4) {
    return null
  }

  const [date, time, target, level] = parts
...
function SourceOriginChip({ origin }: Readonly<{ origin: SourceOrigin }>) {
  return (
    <span
      className={cn(
...
function GridLabel({
  label,
  meta,
}: Readonly<{ label: string; meta?: GridEntryMeta }>) {
...
export function DebugPanel() { // NOSONAR - dev-only diagnostics panel with intentionally dense UI
  const [open, setOpen] = React.useState(false)
  const [panelSide, setPanelSide] = React.useState<DebugPanelSide>(readStoredPanelSide)
  const [activeTab, setActiveTab] = React.useState<DebugPanelTab>(readStoredPanelTab)
  const [attached, setAttached] = React.useState(readStoredAttached)
  ...
  React.useEffect(() => {
    function refreshSystemDefaults() {
      ...
      setLocaleDefaults({
        ...
        region: (() => {
          try {
            return new Intl.Locale(resolved.locale).region ?? null
          } catch {
            return null
          }
        })(),
              ...
              <div className="space-y-1.5">
                {([
                  ...
                  >
                    ...
                    <Button variant="ghost" size="sm" disabled={!value} onClick={() => value && copyDebugText(value).catch(() => undefined)}>
                      <CopyIcon className="size-3.5" />
                      Copy
                    </Button>
                  </div>
